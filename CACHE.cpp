        ...
        if (L2 != nullptr)
        {
         ...
         cout <<  "  L2  misses = " << " READS = " << readm2 << " WRITE = " << writem2 << "\n"
             << "   hit rate = "
               ...
               cout << "\n";
            cout << "  RAM fetches  = " << (readm2 + writem2) << "\n";
             cout << "  Overall rate  = "
...
int main(int argc, char* argv[])
{
    ...
    unordered_map<long long, queue<int>> dummy  ;  // dummy use for LRU/FIFO/LFU 

    for (int i = 0 ; i < trace.size() ; i++)
    {
        long long a = trace[i].addr;
         check = trace[i].op ; 
         lruL1.access(a ,i, dummy);
         fifoL1.access(a, i, dummy);
         optL1.access(a,i, nxtUse);
         lfuL1.access(a , i ,dummy );
    }
